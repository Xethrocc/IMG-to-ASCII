...
-- Uses JuicyPixels' built-in convertRGB8 to handle all pixel formats,
-- then applies standard luminance formula.
toGrayscale :: DynamicImage -> Image Pixel8
toGrayscale dynImg =
  let rgb8 = convertRGB8 dynImg
  in pixelMap luminanceRGB8 rgb8
  where
    -- Standard luminance: 0.2126*R + 0.7152*G + 0.0722*B
    luminanceRGB8 :: PixelRGB8 -> Pixel8
    luminanceRGB8 (PixelRGB8 r g b) =
      round (0.2126 * fromIntegral r + 0.7152 * fromIntegral g + 0.0722 * fromIntegral b :: Double)
